    ...
    public function grantAccessToken(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($requestedScope) {
            ...
            if ($availableScope) {
				...
				if (!$this->scopeUtil->scopeAllowed($grantType->getUserId(), $requestedScope)) {
					$response->setError(403, 'unauthorized_scope', 'The scope requested is not allowed for this user');

					return false;
                // validate the client has access to this scope
				} elseif ($clientScope = $this->clientStorage->getClientScope($clientId)) {
