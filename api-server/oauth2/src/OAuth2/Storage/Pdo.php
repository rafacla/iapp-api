    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function __construct($connection, $config = array())
    {
        ...
        $this->config = array_merge(array(
            ...
            'user_table' => 'register_users',
            'jwt_table'  => 'oauth_jwt',
            'jti_table'  => 'oauth_jti',
            'scope_table'  => 'register_groups',
			'user_scopes_table' => 'register_users_groups',
	...
	public function getUserByID($userID) {
		$stmt = $this->db->prepare($sql = sprintf('SELECT userEmail from %s where userID=\''.$userID.'\'', $this->config['user_table']));
		$stmt->execute();
		...
		$username = $this->getUser($userInfo['userEmail']);
	}
	
    public function getUser($username)
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where userEmail=\''.$username.'\'', $this->config['user_table']));
		$stmt->execute();
		...
		$isAdminSQL = $this->db->prepare($sql = sprintf('SELECT `t1`.`userID`, `t2`.`scope` FROM `%s` `t1` INNER JOIN `%s` `t2` ON `t1`.`groupID` = `t2`.`groupID` WHERE `t1`.`userID` = \''.$userInfo['userID'].'\'', $this->config['user_scopes_table'],$this->config['scope_table']));
		$isAdminSQL->execute();
		
		if($isAdminSQL->fetch(\PDO::FETCH_ASSOC)) {
			$admin = true;
		} else {
			$admin = false;
		}
	...
	/* ScopeInterface */
    public function scopeAllowed($userID, $requiredScope)
    {
		$scope = str_replace("%20"," ",$requiredScope);
        $scope = explode(' ', $scope);
        $whereIn = "'".implode("','", $scope)."'";
        $isAdminSQL = $this->db->prepare($sql = sprintf("SELECT `t1`.`userID`, `t2`.`groupID`,`t2`.`scope` FROM `%s` `t1` INNER JOIN `%s` `t2` ON `t1`.`groupID`= `t2`.`groupID` WHERE userID='%s' AND scope='%s'", $this->config['user_scopes_table'],$this->config['scope_table'],$userID,$requiredScope));
		$isAdminSQL->execute();
